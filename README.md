...
Packets are classified as either inbound or outbound packets. The descriptions here refer to "inbound" and "outbound" from the client's point of view. The first byte is split into two fields of two and six bits respectively. The first field contains an ID for the type of packet sent, and the second contains the ID of the graph whose information was sent, if applicable. The packet payload descriptions are written in terms of C standard integer typedefs and C bitfields.
