...
void Graph::reconnect() {
    ...
    m_outPacket.ID = k_outListPacket;

    if (!sendData(&m_outPacket.ID, sizeof(m_outPacket.ID))) {
        QMessageBox::critical(m_window, QObject::tr("Connection Error"),
                              QObject::tr(
        ...
        if (m_state == ReceiveState::ID) {
            if (m_dataSocket->bytesAvailable() >= 1) {
                ...
                if (!recvFailed) {
                    ...
                    switch (packetID(id)) {
                    case k_inDataPacket:
                        m_inDataPacket.ID = id;
                        m_state = ReceiveState::Data;
                        break;
                    case k_inListPacket:
                        m_inListPacket.ID = id;
            ...
            if (static_cast<quint64>(m_dataSocket->bytesAvailable()) >=
                sizeof(m_inDataPacket.x) + sizeof(m_inDataPacket.y)) {
                recvFailed =
                    !recvData(&m_inDataPacket.x, sizeof(m_inDataPacket.x));

                if (!recvFailed) {
                    recvFailed = !recvData(&m_inDataPacket.y,
                                           sizeof(m_inDataPacket.y));
                    if (!recvFailed) {
            ...
            if (m_dataSocket->bytesAvailable() >= 1) {
                recvFailed =
                    !recvData(&m_inListPacket.length,
                              sizeof(m_inListPacket.length));
                if (!recvFailed) {
                    m_inListPacket.name.resize(m_inListPacket.length);
            ...
            if (m_dataSocket->bytesAvailable() >= m_inListPacket.length) {
                recvFailed = !recvData(&m_inListPacket.name[0],
                                       m_inListPacket.length);
                if (!recvFailed) {
            ...
            if (m_dataSocket->bytesAvailable() >= 1) {
                recvFailed = !recvData(&m_inListPacket.eof, 1);
                if (!recvFailed) {
            ...
            std::memcpy(&xtmp, &m_inDataPacket.x, sizeof(m_inDataPacket.x));
            xtmp = qFromBigEndian<qint64>(xtmp);
            std::memcpy(&m_inDataPacket.x, &xtmp, sizeof(m_inDataPacket.x));

            // Convert endianness of y component
            std::memcpy(&ytmp, &m_inDataPacket.y, sizeof(m_inDataPacket.y));
            ytmp = qFromBigEndian<qint32>(ytmp);
            std::memcpy(&m_inDataPacket.y, &ytmp, sizeof(m_inDataPacket.y));

            // Set time offset based on remote clock
            if (m_startTime == 0) {
                m_startTime = m_inDataPacket.x;
            }
             ...
             */
            uint64_t x = m_inDataPacket.x - m_startTime;
            float y = m_inDataPacket.y;
            addData(graphID(m_inDataPacket.ID), std::make_pair(x / 1000.f, y));
            /* ========================================= */
            ...
            auto i = std::find_if(m_graphNames.begin(), m_graphNames.end(),
                                  [this] (auto& pair) {
                return m_inListPacket.name == pair.second;
            });
...
void Graph::sendGraphChoices() {
    ...
    for (unsigned int i = 0; i < m_graphNames.size(); i++) {
        ...
        if (m_curSelect & (1 << i)) {
            m_outPacket.ID = k_outConnectPacket | m_graphNames[i].first;
        }
        else {
            // Tell server to stop sending stream
            m_outPacket.ID = k_outDisconnectPacket | m_graphNames[i].first;
        }

        if (!sendData(&m_outPacket, sizeof(m_outPacket))) {
            QMessageBox::critical(m_window,
                                  QObject::tr("Connection Error"),
                                  QObject::tr(
