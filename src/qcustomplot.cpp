...
int QCPMarginGroup::commonMargin(QCP::MarginSide side) const {
    ...
    for (int i = 0; i < elements.size(); ++i) {
        ...
        int m = qMax(elements.at(i)->calculateAutoMargin(
                         side),
                     QCP::getMarginValue(elements.at(i)->minimumMargins(),
                                         side));
        if (m > result) {
            result = m;
        }
    ...
    if (phase == upMargins) {
        if (mAutoMargins != QCP::msNone) {
            ...
            foreach(QCP::MarginSide side,
                    QList<QCP::MarginSide>() << QCP::msLeft << QCP::msRight << QCP::msTop <<
                    QCP::msBottom)
            {
                if (mAutoMargins.testFlag(side)) { // this side's margin shall be calculated automatically
                    ...
                    if (QCP::getMarginValue(newMargins,
                        ...
                        QCP::setMarginValue(newMargins,
                                            side,
                                            QCP::getMarginValue(mMinimumMargins,
                                                                side));
                    }
                }
            }
        ...
        while (!unfinishedSections.isEmpty() &&
            ...
            if (nextMax < nextMaxLimit) { // next maximum is actually hit, move forward to that point and fix the size of that section
                for (int i = 0; i < unfinishedSections.size(); ++i) {
                    ...
                    freeSize -= nextMax * stretchFactors.at(unfinishedSections.at(
                                                                i));
                }
...
QSize QCPLayoutGrid::minimumSizeHint() const {
    ...
    result.rwidth() +=
        qMax(0,
             columnCount() - 1) * mColumnSpacing + mMargins.left() +
        mMargins.right();
    result.rheight() +=
        qMax(0,
    ...
    for (int row = 0; row < rowCount(); ++row) {
        for (int col = 0; col < columnCount(); ++col) {
            if (mElements.at(row).at(col)) {
                ...
                QSize final (max.width() <
                             QWIDGETSIZE_MAX ? max.width() : maxHint.width(),
                             max.height() <
                             QWIDGETSIZE_MAX ? max.height() : maxHint.height());
...
void QCPLayoutInset::updateLayout() {
    for (int i = 0; i < mElements.size(); ++i) {
        ...
        finalMinSize.setWidth(mElements.at(
                                  i)->minimumSize().width() > 0 ? mElements.at(
                                  i)->minimumSize().width() : minSizeHint.width());
        finalMinSize.setHeight(mElements.at(
                                   i)->minimumSize().height() > 0 ? mElements.at(
                                   i)->minimumSize().height() : minSizeHint.height());
        finalMaxSize.setWidth(mElements.at(
                                  i)->maximumSize().width() < QWIDGETSIZE_MAX ? mElements.at(
                                  i)->maximumSize().width() : maxSizeHint.width());
        finalMaxSize.setHeight(mElements.at(
                                   i)->maximumSize().height() < QWIDGETSIZE_MAX ? mElements.at(
                                   i)->maximumSize().height() : maxSizeHint.height());
        if (mInsetPlacement.at(i) == ipFree) {
            insetRect = QRect(rect().x() + rect().width() * mInsetRect.at(
                                  i).x(),
                              rect().y() + rect().height() * mInsetRect.at(
                                  i).y(),
            ...
            if (al.testFlag(Qt::AlignLeft)) {
                ...
                insetRect.moveLeft(
                    rect().x() + rect().width() * 0.5 - finalMinSize.width() *
            ...
            if (al.testFlag(Qt::AlignTop)) {
                ...
                insetRect.moveTop(
                    rect().y() + rect().height() * 0.5 - finalMinSize.height() *
...
void QCPLineEnding::draw(QCPPainter* painter,
    ...
    {
        painter->drawLine(
            (pos + widthVec).toPointF(), (pos - widthVec).toPointF());
        break;
    }
    case esHalfBar:
    {
        ...
        if (mZeroLinePen.style() != Qt::NoPen &&
            ...
            for (int i = lowTick; i <= highTick; ++i) {
                if (qAbs(mParentAxis->mTickVector.at(i)) < epsilon) {
                    zeroLineIndex = i;
                    t =
                        mParentAxis->coordToPixel(mParentAxis->mTickVector.at(i)); // x
        ...
        if (mZeroLinePen.style() != Qt::NoPen &&
            ...
            for (int i = lowTick; i <= highTick; ++i) {
                if (qAbs(mParentAxis->mTickVector.at(i)) < epsilon) {
                    zeroLineIndex = i;
                    t =
                        mParentAxis->coordToPixel(mParentAxis->mTickVector.at(i)); // y
...
void QCPAxis::rescale(bool onlyVisiblePlottables) {
    ...
    for (int i = 0; i < p.size(); ++i) {
        ...
        if (mScaleType == stLogarithmic) {
            signDomain =
                (mRange.upper <
                 0 ? QCPAbstractPlottable::sdNegative : QCPAbstractPlottable::
                 sdPositive);
        }
    ...
    if (orientation() == Qt::Horizontal) {
        if (mScaleType == stLinear) {
            ...
            if (value >= 0 && mRange.upper < 0) { // invalid value for logarithmic scale, just draw it outside visible range
                return !mRangeReversed ? mAxisRect->right() +
                       200 : mAxisRect->left() - 200;
            }
            else if (value <= 0 && mRange.upper > 0) { // invalid value for logarithmic scale, just draw it outside visible range
                return !mRangeReversed ? mAxisRect->left() -
                       200 : mAxisRect->right() + 200;
            }
            else {
                if (!mRangeReversed) {
                    return baseLog(value / mRange.lower) / baseLog(
                        mRange.upper / mRange.lower) * mAxisRect->width() +
                           mAxisRect->left();
                }
                else {
                    return baseLog(mRange.upper / value) / baseLog(
                        mRange.upper / mRange.lower) * mAxisRect->width() +
                           mAxisRect->left();
                }
            }
        ...
        if (mScaleType == stLinear) {
            ...
            if (value >= 0 && mRange.upper < 0) { // invalid value for logarithmic scale, just draw it outside visible range
                return !mRangeReversed ? mAxisRect->top() -
                       200 : mAxisRect->bottom() + 200;
            }
            else if (value <= 0 && mRange.upper > 0) { // invalid value for logarithmic scale, just draw it outside visible range
                return !mRangeReversed ? mAxisRect->bottom() +
                       200 : mAxisRect->top() - 200;
            }
            else {
                if (!mRangeReversed) {
...
void QCPAxis::setupTickVectors() {
    ...
    if (mSubTickCount > 0) {
        ...
        int lowTick = mLowestVisibleTick >
                      0 ? mLowestVisibleTick - 1 : mLowestVisibleTick;
        int highTick = mHighestVisibleTick <
                       mTickVector.size() - 1 ? mHighestVisibleTick +
                       1 : mHighestVisibleTick;
    ...
    if (mAutoTickLabels) {
        ...
        if (mTickLabelType == ltNumber) {
            for (int i = mLowestVisibleTick; i <= mHighestVisibleTick; ++i) {
                mTickVectorLabels[i] = mParentPlot->locale().toString(mTickVector.at(
                                                                          i),
            ...
            for (int i = mLowestVisibleTick; i <= mHighestVisibleTick; ++i) {
                ...
                mTickVectorLabels[i] =
                    mParentPlot->locale().toString(QDateTime::fromTime_t(
                                                       mTickVector
                                                       .at(
                                                           i)).toTimeSpec(
                                                       mDateTimeSpec),
...
#else
                mTickVectorLabels[i] =
                    mParentPlot->locale().toString(QDateTime::fromMSecsSinceEpoch(
                                                       mTickVector
                                                       .
                                                       at(
                                                           i)
...
void QCPAxisPainterPrivate::draw(QCPPainter* painter) {
    ...
    QPoint origin;
    switch (type) {
    case QCPAxis::atLeft:   origin = axisRect.bottomLeft() + QPoint(-offset, 0);
        break;
    case QCPAxis::atRight:  origin = axisRect.bottomRight() +
                                     QPoint(+offset, 0); break;
    case QCPAxis::atTop:    origin = axisRect.topLeft()    + QPoint(0, -offset);
        break;
    case QCPAxis::atBottom: origin = axisRect.bottomLeft() + QPoint(0, +offset);
        break;
    }
    ...
    if (!tickPositions.isEmpty()) {
        ...
        int tickDir =
            (type == QCPAxis::atBottom || type == QCPAxis::atRight) ? -1 : 1;       // direction of ticks ("inward" is right for left axis and left for right axis)
        if (QCPAxis::orientation(type) == Qt::Horizontal) {
            for (int i = 0; i < tickPositions.size(); ++i) {
                painter->drawLine(QLineF(tickPositions.at(i) + xCor,
                                         origin.y() - tickLengthOut * tickDir +
                                         yCor,
                                         tickPositions.at(i) + xCor,
                                         origin.y() + tickLengthIn * tickDir +
                                         yCor));
            }
        }
        else {
            for (int i = 0; i < tickPositions.size(); ++i) {
                painter->drawLine(QLineF(origin.x() - tickLengthOut * tickDir +
                                         xCor, tickPositions.at(i) + yCor,
                                         origin.x() + tickLengthIn * tickDir +
                                         xCor,
        ...
        if (QCPAxis::orientation(type) == Qt::Horizontal) {
            for (int i = 0; i < subTickPositions.size(); ++i) {
                painter->drawLine(QLineF(subTickPositions.at(i) + xCor,
                                         origin.y() - subTickLengthOut *
                                         tickDir + yCor,
                                         subTickPositions.at(i) + xCor,
                                         origin.y() + subTickLengthIn *
                                         tickDir + yCor));
            }
        }
        else {
            for (int i = 0; i < subTickPositions.size(); ++i) {
                painter->drawLine(QLineF(origin.x() - subTickLengthOut *
                                         tickDir + xCor,
                                         subTickPositions.at(i) + yCor,
                                         origin.x() + subTickLengthIn *
                                         tickDir + xCor,
                                         subTickPositions.at(i) + yCor));
            }
        }
    ...
    if (lowerEnding.style() != QCPLineEnding::esNone) {
        lowerEnding.draw(painter, QVector2D(
                             baseLine.p1()) - baseLineVector.normalized() * lowerEnding.realLength() *
                         (lowerEnding.inverted() ? -1 : 1), -baseLineVector);
    }
    if (upperEnding.style() != QCPLineEnding::esNone) {
        upperEnding.draw(painter, QVector2D(
                             baseLine.p2()) + baseLineVector.normalized() * upperEnding.realLength() *
    ...
    if (!tickLabels.isEmpty()) {
        ...
        for (int i = 0; i < maxLabelIndex; ++i) {
            placeTickLabel(painter, tickPositions.at(
                               i), distanceToAxis, tickLabels.at(
                               i), &tickLabelsSize);
        }
        if (tickLabelSide == QCPAxis::lsOutside) {
            margin +=
                (QCPAxis::orientation(type) ==
                 Qt::Horizontal) ? tickLabelsSize.height() : tickLabelsSize.
                width();
        }
    }
    if (tickLabelSide == QCPAxis::lsInside) {
    ...
    if (!label.isEmpty()) {
        ...
        labelBounds = painter->fontMetrics().boundingRect(0,
                                                          0,
                                                          0,
                                                          0,
                                                          Qt::TextDontClip,
        ...
        if (type == QCPAxis::atLeft) {
            QTransform oldTransform = painter->transform();
            painter->translate(
                (origin.x() - margin - labelBounds.height()), origin.y());
            painter->rotate(-90);
            painter->drawText(0, 0, axisRect.height(),
            ...
            painter->drawText(origin.x(), origin.y() + margin,
                              axisRect.width(),
                              labelBounds.height(), Qt::TextDontClip | Qt::AlignCenter,
                              label);
        }
    ...
    int selAxisOutSize = qMax(qMax(tickLengthOut,
                                   subTickLengthOut), selectionTolerance);
    ...
    if (tickLabelSide == QCPAxis::lsOutside) {
        selTickLabelSize =
            (QCPAxis::orientation(type) ==
             Qt::Horizontal ? tickLabelsSize.height() : tickLabelsSize.width());
        selTickLabelOffset =
    ...
    int selLabelOffset =
        ...
        (!tickLabels.isEmpty() &&
         tickLabelSide ==
         QCPAxis::lsOutside ? tickLabelPadding + selTickLabelSize : 0) +
        labelPadding;
    if (type == QCPAxis::atLeft) {
        ...
        mTickLabelsSelectionBox.setCoords(
            ...
            axisRect.top(), origin.x() - selTickLabelOffset, axisRect.bottom());
        mLabelSelectionBox.setCoords(
            origin.x() - selLabelOffset - selLabelSize,
    ...
    if (tickLabelSide == QCPAxis::lsOutside) {
        ...
        if (!tickLabels.isEmpty()) {
            for (int i = 0; i < tickLabels.size(); ++i) {
                getMaxTickLabelSize(tickLabelFont, tickLabels.at(
                                        i), &tickLabelsSize);
            }
            result += QCPAxis::orientation(type) ==
                      Qt::Horizontal ? tickLabelsSize.height() : tickLabelsSize.
                      width();
...
void QCPAxisPainterPrivate::placeTickLabel(QCPPainter* painter,
    ...
    QPointF labelAnchor;
    switch (type) {
    case QCPAxis::atLeft:   labelAnchor = QPointF(
            axisRect.left() - distanceToAxis - offset, position); break;
    case QCPAxis::atRight:  labelAnchor = QPointF(
            axisRect.right() + distanceToAxis + offset, position); break;
    case QCPAxis::atTop:    labelAnchor = QPointF(position,
                                                  axisRect.top() - distanceToAxis -
                                                  offset); break;
    case QCPAxis::atBottom: labelAnchor = QPointF(position,
                                                  axisRect.bottom() + distanceToAxis +
                                                  offset); break;
    }
    if (mParentPlot->plottingHints().testFlag(QCP::phCacheLabels) &&
        ...
        if (!mLabelCache.contains(text)) { // no cached label exists, create it
            ...
            newCachedLabel->pixmap =
                QPixmap(labelData.rotatedTotalBounds.size());
                                          ...
                                          const TickLabelData& labelData) const
{
    ...
    if (!labelData.expPart.isEmpty()) { // indicator that beautiful powers must be used
        ...
        painter->drawText(
            labelData.baseBounds.width() + 1, 0,
...
QCPAxisPainterPrivate::TickLabelData QCPAxisPainterPrivate::getTickLabelData(
    ...
    if (useBeautifulPowers) {
        ...
        result.baseBounds = QFontMetrics(result.baseFont).boundingRect(0,
                                                                       0,
                                                                       0,
                                                                       0,
                                                                       Qt::TextDontClip,
                                                                       result.basePart);
        result.expBounds = QFontMetrics(result.expFont).boundingRect(0,
                                                                     0,
                                                                     0,
                                                                     0,
                                                                     Qt::TextDontClip,
    ...
    if ((type == QCPAxis::atLeft && tickLabelSide == QCPAxis::lsOutside) ||
        ...
        if (doRotation) {
            if (tickLabelRotation > 0) {
                ...
                y = flip ? -labelData.totalBounds.width() /
                    2.0 : -qSin(radians) * labelData.totalBounds.width() - qCos(
                ...
                y = flip ? +labelData.totalBounds.width() /
                    2.0 : +qSin(-radians) * labelData.totalBounds.width() -
                    qCos(
                    -radians) * labelData.totalBounds.height() / 2.0;
            }
        }
        ...
        if (doRotation) {
            if (tickLabelRotation > 0) {
                ...
                y = flip ? -labelData.totalBounds.width() /
                    2.0 : -qCos(radians) * labelData.totalBounds.height() / 2.0;
            }
            else {
                x = 0;
                y = flip ? +labelData.totalBounds.width() /
                    2.0 : -qCos(-radians) * labelData.totalBounds.height() /
                    2.0;
            }
        }
        else {
    ...
    if (foundRange) {
        ...
        if (!QCPRange::validRange(newRange)) { // likely due to range being zero (plottable has only constant data in this axis dimension), shift current range to at least center the plottable
            ...
            if (keyAxis->scaleType() == QCPAxis::stLinear) {
                ...
                newRange.lower = center / qSqrt(
                    keyAxis->range().upper / keyAxis->range().lower);
                newRange.upper = center * qSqrt(
                    keyAxis->range().upper / keyAxis->range().lower);
            }
        }
...
void QCPAbstractPlottable::rescaleValueAxis(bool onlyEnlarge) const {
    ...
    if (!valueAxis) {
        qDebug() << Q_FUNC_INFO << "invalid value axis"; return;
    }
    ...
    if (foundRange) {
        ...
        if (!QCPRange::validRange(newRange)) { // likely due to range being zero (plottable has only constant data in this axis dimension), shift current range to at least center the plottable
            ...
            if (valueAxis->scaleType() == QCPAxis::stLinear) {
                ...
                newRange.lower = center / qSqrt(
                    valueAxis->range().upper / valueAxis->range().lower);
                newRange.upper = center * qSqrt(
                    valueAxis->range().upper / valueAxis->range().lower);
            }
        }
...
bool QCPAbstractPlottable::addToLegend() {
    ...
    if (!mParentPlot->legend->hasItemWithPlottable(this)) {
        mParentPlot->legend->addItem(new QCPPlottableLegendItem(mParentPlot->
                                                                legend, this));
        return true;
    }
    else {
...
const QPointF QCPAbstractPlottable::coordsToPixels(double key,
    ...
    if (keyAxis->orientation() == Qt::Horizontal) {
        return QPointF(keyAxis->coordToPixel(key), valueAxis->coordToPixel(
                           value));
    }
    else {
        return QPointF(valueAxis->coordToPixel(value), keyAxis->coordToPixel(
                           key));
    }
}
 ...
 */
void QCPAbstractPlottable::applyDefaultAntialiasingHint(QCPPainter* painter)
const {
 ...
 */
void QCPAbstractPlottable::applyScattersAntialiasingHint(QCPPainter* painter)
const {
 ...
 */
void QCPAbstractPlottable::applyErrorBarsAntialiasingHint(QCPPainter* painter)
const {
...
double QCPAbstractItem::rectSelectTest(const QRectF& rect,
    ...
    // distance to border:
    QList<QLineF> lines;
    lines << QLineF(rect.topLeft(), rect.topRight()) << QLineF(
        rect.bottomLeft(), rect.bottomRight())
          << QLineF(rect.topLeft(), rect.bottomLeft()) << QLineF(
...
QCPItemAnchor* QCPAbstractItem::createAnchor(const QString& name,
    ...
    QCPItemAnchor* newAnchor = new QCPItemAnchor(mParentPlot,
                                                 this,
                                                 name,
                                                 anchorId);
...
QCPAbstractPlottable* QCustomPlot::plottableAt(const QPointF& pos,
    ...
    foreach(QCPAbstractPlottable * plottable, mPlottables)
    {
        ...
        if ((plottable->keyAxis()->axisRect()->rect() &
             plottable->valueAxis()->axisRect()->rect()).contains(pos.toPoint()))
        {                                                                                                                // only consider clicks inside the rect that is spanned by the plottable's key/value axes
...
bool QCustomPlot::addLayer(const QString& name,
    ...
    mLayers.insert(
        otherLayer->index() + (insertMode == limAbove ? 1 : 0), newLayer);
    updateLayerIndices();
    return true;
}
...
bool QCustomPlot::removeLayer(QCPLayer* layer) {
    ...
    QCPLayer* targetLayer =
        isFirstLayer ? mLayers.at(removedIndex + 1) : mLayers.at(
            removedIndex - 1);
...
void QCustomPlot::replot(QCustomPlot::RefreshPriority refreshPriority) {
    ...
    mPaintBuffer.fill(
        ...
        Qt::SolidPattern ? mBackgroundBrush.color() : Qt::transparent);
    QCPPainter painter;
    ...
    if ((mMousePressPos - event->pos()).manhattanLength() < 5) { // determine whether it was a click operation
        if (event->button() == Qt::LeftButton) {
            ...
            if (clickedLayerable &&
                ...
                bool selChanged = false;
                clickedLayerable->selectEvent(event,
                                              additive,
                                              details,
                                              &selChanged);
                selectionStateChanged |= selChanged;
        ...
        QVariant details;
        QCPLayerable* clickedLayerable = layerableAt(
            event->pos(), false, &details);                                      // for these signals, selectability is ignored, that's why we call this again with onlySelectable set to false
        if (QCPAbstractPlottable* ap =
                qobject_cast<QCPAbstractPlottable*>(clickedLayerable)) {
...
QCPLayerable* QCustomPlot::layerableAt(const QPointF& pos,
    ...
    for (int layerIndex = mLayers.size() - 1; layerIndex >= 0; --layerIndex) {
        ...
        for (int i = layerables.size() - 1; i >= 0; --i) {
            ...
            QVariant details;
            double dist = layerables.at(i)->selectTest(pos,
                                                       onlySelectable,
                                                       &details);
...
QPixmap QCustomPlot::toPixmap(int width, int height, double scale) {
    ...
    result.fill(
        mBackgroundBrush.style() ==
        Qt::SolidPattern ? mBackgroundBrush.color() : Qt::transparent);                                   // if using non-solid pattern, make transparent now and draw brush pattern later
    QCPPainter painter;
...
void QCPColorGradient::colorize(const double* data,
    ...
    if (!logarithmic) {
        ...
        if (mPeriodic) {
            for (int i = 0; i < n; ++i) {
                int index =
                    (int) ((data[dataIndexFactor * i] - range.lower) *
...
QCPAxis* QCPAxisRect::rangeDragAxis(Qt::Orientation orientation) {
    return orientation ==
           Qt::Horizontal ? mRangeDragHorzAxis.data() : mRangeDragVertAxis.data();
}
...
QCPAxis* QCPAxisRect::rangeZoomAxis(Qt::Orientation orientation) {
    return orientation ==
           Qt::Horizontal ? mRangeZoomHorzAxis.data() : mRangeZoomVertAxis.data();
}
...
double QCPAxisRect::rangeZoomFactor(Qt::Orientation orientation) {
    return orientation ==
           Qt::Horizontal ? mRangeZoomFactorHorz : mRangeZoomFactorVert;
}
...
/* inherits documentation from base class */
void QCPAbstractLegendItem::applyDefaultAntialiasingHint(QCPPainter* painter)
const {
...
QPen QCPPlottableLegendItem::getIconBorderPen() const {
    return mSelected ? mParentLegend->selectedIconBorderPen() : mParentLegend->
           iconBorderPen();
}
...
QSize QCPPlottableLegendItem::minimumSizeHint() const {
    ...
    textRect = fontMetrics.boundingRect(0, 0, 0,
                                        iconSize.height(), Qt::TextDontClip,
                                        mPlottable->name());
    result.setWidth(
        iconSize.width() + mParentLegend->iconTextPadding() + textRect.width() + mMargins.left() +
        mMargins.right());
    result.setHeight(qMax(textRect.height(),
                          iconSize.height()) + mMargins.top() +
                     mMargins.bottom());
    return result;
}
...
void QCPLegend::selectEvent(QMouseEvent* event,
    ...
    if (details.value<SelectablePart>() == spLegendBox &&
        ...
        SelectableParts selBefore = mSelectedParts;
        setSelectedParts(
            additive ? mSelectedParts ^ spLegendBox : mSelectedParts |
...
void QCPColorScale::setType(QCPAxis::AxisType type) {
    ...
    if (mType != type) {
        ...
        if (mColorAxis) {
            ...
            disconnect(mColorAxis.data(), SIGNAL(rangeChanged(
                                                     QCPRange)), this,
        ...
        foreach(QCPAxis::AxisType atype,
                QList<QCPAxis::AxisType>() << QCPAxis::atLeft << QCPAxis::atRight << QCPAxis::atBottom <<
                QCPAxis::atTop)
        {
        ...
        connect(mColorAxis.data(), SIGNAL(rangeChanged(QCPRange)), this,
                SLOT(setDataRange(QCPRange)));
        connect(mColorAxis.data(), SIGNAL(scaleTypeChanged(
                                              QCPAxis::ScaleType)), this,
                SLOT(setDataScaleType(QCPAxis::ScaleType)));
        mAxisRect.data()->setRangeDragAxes(QCPAxis::orientation(
                                               mType) == Qt::Horizontal ? mColorAxis.data() : 0,
                                           QCPAxis::orientation(
                                               mType) ==
                                           Qt::Vertical ? mColorAxis.data() : 0);
    }
}
    ...
    if (haveRange) {
        if (!QCPRange::validRange(newRange)) { // likely due to range being zero (plottable has only constant data in this dimension), shift current range to at least center the data
            ...
            if (mDataScaleType == QCPAxis::stLinear) {
                ...
                newRange.lower = center / qSqrt(
                    mDataRange.upper / mDataRange.lower);
                newRange.upper = center * qSqrt(
                    mDataRange.upper / mDataRange.lower);
            }
        }
    ...
    case upMargins:
    {
        if (mType == QCPAxis::atBottom || mType == QCPAxis::atTop) {
            setMaximumSize(QWIDGETSIZE_MAX,
                           mBarWidth + mAxisRect.data()->margins().top() + mAxisRect.data()->margins().bottom() + margins().top() +
                           margins().bottom());
            setMinimumSize(0,
                           mBarWidth + mAxisRect.data()->margins().top() + mAxisRect.data()->margins().bottom() + margins().top() +
                           margins().bottom());
            ...
            setMaximumSize(
                mBarWidth + mAxisRect.data()->margins().left() + mAxisRect.data()->margins().right() + margins().left() + margins().right(),
                QWIDGETSIZE_MAX);
            setMinimumSize(
                mBarWidth + mAxisRect.data()->margins().left() + mAxisRect.data()->margins().right() + margins().left() + margins().right(),
                0);
...
QCPColorScaleAxisRectPrivate::QCPColorScaleAxisRectPrivate(
    ...
    foreach(QCPAxis::AxisType type,
            QList<QCPAxis::AxisType>() << QCPAxis::atBottom << QCPAxis::atTop << QCPAxis::atLeft <<
            QCPAxis::atRight)
    {
        axis(type)->setVisible(true);
        axis(type)->grid()->setVisible(false);
        axis(type)->setPadding(0);
        connect(axis(type), SIGNAL(selectionChanged(
                                       QCPAxis::SelectableParts)), this,
                SLOT(axisSelectionChanged(QCPAxis::SelectableParts)));
        connect(axis(type), SIGNAL(selectableChanged(
                                       QCPAxis::SelectableParts)), this,
    ...
    connect(axis(QCPAxis::atLeft), SIGNAL(scaleTypeChanged(
                                              QCPAxis::ScaleType)),
            axis(QCPAxis::atRight),
            SLOT(setScaleType(QCPAxis::ScaleType)));
    connect(axis(QCPAxis::atRight), SIGNAL(scaleTypeChanged(
                                               QCPAxis::ScaleType)),
            axis(QCPAxis::atLeft),
            SLOT(setScaleType(QCPAxis::ScaleType)));
    connect(axis(QCPAxis::atBottom), SIGNAL(scaleTypeChanged(
                                                QCPAxis::ScaleType)),
            axis(QCPAxis::atTop),
            SLOT(setScaleType(QCPAxis::ScaleType)));
    connect(axis(QCPAxis::atTop), SIGNAL(scaleTypeChanged(
                                             QCPAxis::ScaleType)),
    ...
    if (mParentColorScale->mType == QCPAxis::atBottom ||
        ...
        for (int y = 0; y < h; ++y) {
            ...
            const QRgb lineColor =
                mParentColorScale->mGradient.color(data[h - 1 - y],
                                                   QCPRange(0, n - 1));
            for (int x = 0; x < w; ++x) {
                pixels[x] = lineColor;
            }
            ...
            QCPAxis::atRight)
    {
        ...
        if (axis(type)->selectableParts().testFlag(QCPAxis::spAxis)) {
            if (selectedParts.testFlag(QCPAxis::spAxis)) {
                axis(type)->setSelectedParts(axis(
                                                 type)->selectedParts() |
                                             QCPAxis::spAxis);
            }
            else {
            ...
            QCPAxis::atRight)
    {
        ...
        if (axis(type)->selectableParts().testFlag(QCPAxis::spAxis)) {
            if (selectableParts.testFlag(QCPAxis::spAxis)) {
                axis(type)->setSelectableParts(axis(
...
double QCPGraph::selectTest(const QPointF& pos,
    ...
    if (!mKeyAxis || !mValueAxis) {
        qDebug() << Q_FUNC_INFO << "invalid key or value axis"; return -1;
    }
...
void QCPGraph::getLinePlotData(QVector<QPointF>* linePixelData,
    ...
    if (keyAxis->orientation() == Qt::Vertical) {
        for (int i = 0; i < lineData.size(); ++i) {
            (*linePixelData)[i].setX(valueAxis->coordToPixel(lineData.at(i).
                                                             value));
        ...
        for (int i = 0; i < lineData.size(); ++i) {
            ...
            (*linePixelData)[i].setY(valueAxis->coordToPixel(lineData.at(i).
                                                             value));
        }
    }
}
...
void QCPGraph::getImpulsePlotData(QVector<QPointF>* linePixelData,
    ...
    if (keyAxis->orientation() == Qt::Vertical) {
        ...
        for (int i = 0; i < lineData.size(); ++i) {
            ...
            (*linePixelData)[i * 2 + 0].setY(key);
            (*linePixelData)[i * 2 +
                             1].setX(valueAxis->coordToPixel(lineData.at(i).
                                                             value));
            (*linePixelData)[i * 2 + 1].setY(key);
        }
    }
        ...
        for (int i = 0; i < lineData.size(); ++i) {
            ...
            (*linePixelData)[i * 2 + 1].setX(key);
            (*linePixelData)[i * 2 +
                             1].setY(valueAxis->coordToPixel(lineData.at(i).
                                                             value));
        }
    }
}
    ...
    if (mErrorType != etNone) {
        ...
        if (keyAxis->orientation() == Qt::Vertical) {
            for (int i = 0; i < scatterData->size(); ++i) {
                drawError(painter,
                          valueAxis->coordToPixel(scatterData->at(
                                                      i).value),
                          keyAxis->coordToPixel(scatterData->at(
                                                    i)
                                                .key), scatterData->at(i));
            }
        }
        else {
            for (int i = 0; i < scatterData->size(); ++i) {
                drawError(painter, keyAxis->coordToPixel(scatterData->at(
                                                             i).key),
                          valueAxis->coordToPixel(scatterData->at(
                                                      i)
                                                  .value), scatterData->at(i));
            }
        }
    }
        ...
        for (int i = 0; i < scatterData->size(); ++i) {
            if (!qIsNaN(scatterData->at(i).value)) {
                mScatterStyle.drawShape(painter,
                                        valueAxis->coordToPixel(scatterData->at(
                                                                    i).value),
                                        keyAxis->coordToPixel(scatterData->at(i)
                                                              .key));
            }
        }
    }
            ...
            while (i < lineDataSize) {
                if (qIsNaN(lineData->at(i).y()) ||
                    ...
                    painter->drawPolyline(
                        lineData->constData() + segmentStart, i - segmentStart);     // i, because we don't want to include the current NaN point
                    segmentStart = i + 1;
                }
                ++i;
            }
...
void QCPGraph::getPreparedData(QVector<QCPData>* lineData,
    ...
    if (mAdaptiveSampling) {
        int keyPixelSpan = qAbs(keyAxis->coordToPixel(
                                    lower.key()) -
    ...
    if (mAdaptiveSampling && dataCount >= maxCount) { // use adaptive sampling only if there are at least two points per pixel on average
        if (lineData) {
            ...
            int reversedFactor = keyAxis->rangeReversed() !=
                                 (keyAxis->orientation() ==
                                  Qt::Vertical) ? -1 : 1;                                               // is used to calculate keyEpsilon pixel into the correct direction
            int reversedRound = keyAxis->rangeReversed() !=
                                (keyAxis->orientation() ==
                                 Qt::Vertical) ? 1 : 0;                                               // is used to switch between floor (normal) and ceil (reversed) rounding of currentIntervalStartKey
            double currentIntervalStartKey =
                keyAxis->pixelToCoord((int) (keyAxis->coordToPixel(lower.key())
                                             +
                                             reversedRound));
            double lastIntervalEndKey = currentIntervalStartKey;
            double keyEpsilon =
                qAbs(currentIntervalStartKey -
                     keyAxis->pixelToCoord(keyAxis->coordToPixel(
            ...
            while (it != upperEnd) {
                if (it.key() < currentIntervalStartKey + keyEpsilon) { // data point is still within same pixel, so skip it and expand value span of this cluster if necessary
                    ...
                    if (intervalDataCount >= 2) { // last pixel had multiple data points, consolidate them to a cluster
                        ...
                        if (it.key() >
                            currentIntervalStartKey + keyEpsilon * 2) { // new pixel started further away from previous cluster, so make sure the last point of the cluster is at a real data point
                            lineData->append(QCPData(currentIntervalStartKey +
                                                     keyEpsilon * 0.8,
                                                     (it - 1).value().value));
                        }
                    ...
                    currentIntervalStartKey =
                        keyAxis->pixelToCoord((int) (keyAxis->coordToPixel(it.
                                                                           key())
                                                     + reversedRound));
            ...
            if (intervalDataCount >= 2) { // last pixel had multiple data points, consolidate them to a cluster
                ...
                lineData->append(QCPData(currentIntervalStartKey + keyEpsilon *
                                         0.25, minValue));
                lineData->append(QCPData(currentIntervalStartKey + keyEpsilon *
                                         0.75, maxValue));
            }
        ...
        if (scatterData) {
            ...
            if (intervalDataCount >= 2) { // last pixel had multiple data points, consolidate them
                // determine value pixel span and add as many points in interval to maintain certain vertical data density (this is specific to scatter plot):
                double valuePixelSpan = qAbs(valueAxis->coordToPixel(
                                                 minValue) -
                                             valueAxis->coordToPixel(maxValue));
                int dataModulo =
                    qMax(1, qRound(intervalDataCount / (valuePixelSpan / 4.0)));  // approximately every 4 value pixels one data point on average
...
const {
    ...
    if (keyAxis->orientation() == Qt::Horizontal) {
        ...
        (*croppedData)[0].setY(croppedData->at(0).y() + slope *
                               (staticData->first().x() - croppedData->at(
                                    0).x()));
        ...
        (*croppedData)[0].setX(croppedData->at(0).x() + slope *
                               (staticData->first().y() - croppedData->at(
                                    0).y()));
...
double QCPGraph::pointDistance(const QPointF& pixelPoint) const {
    if (mData->isEmpty()) {
        qDebug() << Q_FUNC_INFO << "requested point distance on graph" <<
            mName << "without data";
        return 500;
    }
    if (mData->size() == 1) {
        QPointF dataPoint = coordsToPixels(
            mData->constBegin().key(), mData->constBegin().value().value);
    ...
    if (mLineStyle == lsNone) {
        ...
        double minDistSqr = std::numeric_limits<double>::max();
        QPointF ptA;
        QPointF ptB = coordsToPixels(scatterData->at(0).key, scatterData->at(
                                         0).value);                                 // getScatterPlotData returns in plot coordinates, so transform to pixels
        for (int i = 1; i < scatterData->size(); ++i) {
            ptA = ptB;
            ptB = coordsToPixels(scatterData->at(i).key, scatterData->at(
                                     i).value);
        ...
        if (mLineStyle == lsImpulse) {
            ...
            for (int i = 0; i < lineData->size() - 1; i += 2) { // iterate pairs
                double currentDistSqr = distSqrToLine(lineData->at(
                                                          i), lineData->at(
                                                          i + 1), pixelPoint);
                if (currentDistSqr < minDistSqr) {
                    minDistSqr = currentDistSqr;
...
void QCPCurve::getCurveData(QVector<QPointF>* lineData) const {
    ...
    double rectLeft =
        keyAxis->pixelToCoord(keyAxis->coordToPixel(
                                  keyAxis->range().lower) - strokeMargin *
                              ((keyAxis->orientation() == Qt::Vertical) !=
                               keyAxis->rangeReversed() ? -1 : 1));
    double rectRight =
        keyAxis->pixelToCoord(keyAxis->coordToPixel(
                                  keyAxis->range().upper) + strokeMargin *
                              ((keyAxis->orientation() == Qt::Vertical) !=
                               keyAxis->rangeReversed() ? -1 : 1));
    double rectBottom =
        valueAxis->pixelToCoord(valueAxis->coordToPixel(
                                    valueAxis->range().lower) + strokeMargin *
                                ((valueAxis->orientation() == Qt::Horizontal) !=
                                 valueAxis->rangeReversed() ? -1 : 1));
    double rectTop =
        valueAxis->pixelToCoord(valueAxis->coordToPixel(
                                    valueAxis->range().upper) - strokeMargin *
                                ((valueAxis->orientation() == Qt::Horizontal) !=
                                 valueAxis->rangeReversed() ? -1 : 1));
    int currentRegion;
    ...
    int prevRegion = getRegion(prevIt.value().key,
                               prevIt.value().value, rectLeft, rectTop, rectRight,
                               rectBottom);
    ...
    case 1: // top and left edge
    {
        ...
        if (intersectKey < rectLeft || intersectKey > rectRight) { // doesn't intersect, so must intersect other:
            ...
            intersectValue = otherValue + (value - otherValue) /
                             (key - otherKey) * (intersectKey - otherKey);
        }
        break;
    }
    case 2: // left edge
...
QVector<QPointF> QCPCurve::getOptimizedCornerPoints(int prevRegion,
    ...
    case 1:
    {
        ...
        case 3: { result << coordsToPixels(rectLeft, rectTop) << coordsToPixels(
                      rectLeft,
                      rectBottom); break; }
        case 7: { result << coordsToPixels(rectLeft, rectTop) << coordsToPixels(
                      rectRight,
                      rectTop); break; }
        case 6: { result << coordsToPixels(rectLeft, rectTop) << coordsToPixels(
                      rectLeft,
                      rectBottom); result.append(result.last()); break; }
        case 8: { result << coordsToPixels(rectLeft, rectTop) << coordsToPixels(
                      rectRight,
                      rectTop); result.append(result.last()); break; }
        case 9: { // in this case we need another distinction of cases: segment may pass below or above rect, requiring either bottom right or top left corner points
            if ((value - prevValue) / (key - prevKey) * (rectLeft - key) +
                ...
                result << coordsToPixels(rectLeft, rectTop) << coordsToPixels(
                    rectLeft,
                    rectBottom); result.append(result.last());
                result << coordsToPixels(rectRight, rectBottom);
            }
            else {
                result << coordsToPixels(rectLeft, rectTop) << coordsToPixels(
                    rectRight,
                    rectTop); result.append(result.last());
                result << coordsToPixels(rectRight, rectBottom);
            }
            break;
        }
    ...
    case 2:
    {
        ...
        case 4: { result << coordsToPixels(rectLeft, rectTop); result.append(
                      result.last()); break; }
        case 6: { result << coordsToPixels(rectLeft, rectBottom); result.append(
                      result.last()); break; }
        case 7: { result << coordsToPixels(rectLeft, rectTop); result.append(
                      result.last()); result << coordsToPixels(rectRight,
                                                               rectTop); break;
        }
        case 9: { result << coordsToPixels(rectLeft, rectBottom); result.append(
                      result.last()); result << coordsToPixels(rectRight,
                                                               rectBottom);
                  break; }
        }
    ...
    case 3:
    {
        ...
        case 1: { result <<
                  coordsToPixels(rectLeft, rectBottom) << coordsToPixels(
                      rectLeft,
                      rectTop); break; }
        case 9: { result <<
                  coordsToPixels(rectLeft, rectBottom) << coordsToPixels(
    ...
    case 4:
    {
        ...
        case 2: { result << coordsToPixels(rectLeft, rectTop); result.append(
                      result.last()); break; }
        case 8: { result << coordsToPixels(rectRight, rectTop); result.append(
                      result.last()); break; }
        case 3: { result << coordsToPixels(rectLeft, rectTop); result.append(
                      result.last()); result << coordsToPixels(rectLeft,
                                                               rectBottom);
                  break; }
        case 9: { result << coordsToPixels(rectRight, rectTop); result.append(
                      result.last()); result << coordsToPixels(rectRight,
                                                               rectBottom);
                  break; }
        }
    ...
    case 6:
    {
        ...
        case 3: { result << coordsToPixels(rectLeft, rectBottom); break; }
        case 9: { result << coordsToPixels(rectRight, rectBottom); break; }
        case 2: { result << coordsToPixels(rectLeft, rectBottom); result.append(
                      result.last()); break; }
        case 8: { result << coordsToPixels(rectRight, rectBottom);
                  result.append(result.last()); break; }
        case 1: { result << coordsToPixels(rectLeft, rectBottom); result.append(
                      result.last()); result << coordsToPixels(rectLeft,
                                                               rectTop); break;
        }
        case 7: { result << coordsToPixels(rectRight, rectBottom);
                  result.append(result.last()); result << coordsToPixels(
                      rectRight,
                      rectTop); break; }
        }
        break;
    ...
    case 7:
    {
        ...
        case 1: { result <<
                  coordsToPixels(rectRight, rectTop) << coordsToPixels(
                      rectLeft,
                      rectTop); break; }
        case 9: { result <<
                  coordsToPixels(rectRight, rectTop) << coordsToPixels(
        ...
        case 3: { // in this case we need another distinction of cases: segment may pass below or above rect, requiring either bottom right or top left corner points
            if ((value - prevValue) / (key - prevKey) * (rectRight - key) +
                value < rectBottom) {                                             // segment passes below R
                result << coordsToPixels(rectRight, rectTop) << coordsToPixels(
                    rectRight,
                    rectBottom); result.append(result.last());
    ...
    case 8:
    {
        ...
        case 7: { result << coordsToPixels(rectRight, rectTop); break; }
        case 9: { result << coordsToPixels(rectRight, rectBottom); break; }
        case 4: { result << coordsToPixels(rectRight, rectTop); result.append(
                      result.last()); break; }
        case 6: { result << coordsToPixels(rectRight, rectBottom);
                  result.append(result.last()); break; }
        case 1: { result << coordsToPixels(rectRight, rectTop); result.append(
                      result.last()); result << coordsToPixels(rectLeft,
                                                               rectTop); break;
        }
        case 3: { result << coordsToPixels(rectRight, rectBottom);
                  result.append(result.last()); result << coordsToPixels(
                      rectLeft,
                      rectBottom); break; }
        }
    ...
    case 9:
    {
        ...
        case 3: { result <<
                  coordsToPixels(rectRight, rectBottom) << coordsToPixels(
                      rectLeft,
                      rectBottom); break; }
        case 7: { result <<
                  coordsToPixels(rectRight, rectBottom) << coordsToPixels(
...
void QCPCurve::getTraverseCornerPoints(int prevRegion,
    ...
    case 1:
    {
        ...
        case 9: { beforeTraverse << coordsToPixels(rectLeft, rectTop);
    ...
    case 3:
    {
        ...
        case 7: { beforeTraverse << coordsToPixels(rectLeft, rectBottom);
    ...
    case 7:
    {
        ...
        case 3: { beforeTraverse << coordsToPixels(rectRight, rectTop);
    ...
    {
        switch (currentRegion) {
        case 2: { beforeTraverse << coordsToPixels(rectRight, rectBottom);
                  break; }
        case 1: { beforeTraverse << coordsToPixels(rectRight, rectBottom);
                  afterTraverse << coordsToPixels(rectLeft, rectTop); break; }
        case 4: { beforeTraverse << coordsToPixels(rectRight, rectBottom);
                  break; }
        }
        break;
    }
...
double QCPCurve::pointDistance(const QPointF& pixelPoint) const {
    if (mData->isEmpty()) {
        qDebug() << Q_FUNC_INFO << "requested point distance on curve" <<
            mName << "without data";
        return 500;
    }
    if (mData->size() == 1) {
    ...
    if (index >= 0) {
        ...
        if (baseBars.size() % 2 == 1 && index == (baseBars.size() - 1) / 2) { // is center bar (int division on purpose)
            ...
            if (baseBars.size() % 2 == 0) { // even number of bars
                ...
                result -=
                    getPixelSpacing(baseBars.at(startIndex), keyCoord) * 0.5; // half of middle spacing
            }
            ...
            baseBars.at(index)->getPixelWidth(keyCoord,
                                              lowerPixelWidth,
                                              upperPixelWidth);
...
QCPRange QCPBars::getValueRange(bool& foundRange,
    ...
    QCPBarDataMap::const_iterator it = mData->constBegin();
    while (it != mData->constEnd()) {
        current = it.value().value + getStackedBaseValue(
            it.value().key, it.value().value >= 0);
        if (inSignDomain == sdBoth ||
            (inSignDomain == sdNegative && current < 0) ||
            (inSignDomain == sdPositive && current > 0)) {
            if (current < range.lower || !haveLower) {
...
double QCPStatisticalBox::selectTest(const QPointF& pos,
    ...
    if (mKeyAxis.data()->axisRect()->rect().contains(pos.toPoint())) {
        ...
        if (QCPRange(mMinimum, mMaximum).contains(posValue)) {
            return qAbs(mKeyAxis.data()->coordToPixel(
                            mKey) - mKeyAxis.data()->coordToPixel(posKey));
        }
    }
    return -1;
}
...
void QCPStatisticalBox::drawWhiskers(QCPPainter* painter) const {
    ...
    barMax.setPoints(coordsToPixels(mKey - mWhiskerWidth * 0.5,
                                    mMaximum),
                     coordsToPixels(mKey + mWhiskerWidth * 0.5, mMaximum));
    barMin.setPoints(coordsToPixels(mKey - mWhiskerWidth * 0.5,
                                    mMinimum),
    ...
    if (keySize != mKeySize || valueSize != mValueSize) {
        ...
        if (!mIsEmpty) {
            ...
            if (mData) {
                ...
                qDebug() << Q_FUNC_INFO <<
                    "out of memory for data dimensions " << mKeySize << "*" <<
                    mValueSize;
            }
        }
        else {
...
void QCPColorMap::setColorScale(QCPColorScale* colorScale) {
    if (mColorScale) { // unconnect signals from old color scale
        disconnect(this, SIGNAL(dataRangeChanged(QCPRange)),
                   mColorScale.data(), SLOT(setDataRange(QCPRange)));
        disconnect(this, SIGNAL(dataScaleTypeChanged(
                                    QCPAxis::ScaleType)), mColorScale.data(),
                   SLOT(setDataScaleType(QCPAxis::ScaleType)));
        disconnect(this, SIGNAL(gradientChanged(
                                    QCPColorGradient)), mColorScale.data(),
                   SLOT(setGradient(QCPColorGradient)));
        disconnect(mColorScale.data(), SIGNAL(dataRangeChanged(
                                                  QCPRange)), this,
                   SLOT(setDataRange(QCPRange)));
        disconnect(mColorScale.data(), SIGNAL(gradientChanged(
                                                  QCPColorGradient)), this,
                   SLOT(setGradient(QCPColorGradient)));
    ...
    if (mColorScale) { // connect signals to new color scale
        ...
        connect(this, SIGNAL(dataRangeChanged(QCPRange)),
                mColorScale.data(), SLOT(setDataRange(QCPRange)));
        connect(this, SIGNAL(dataScaleTypeChanged(
                                 QCPAxis::ScaleType)), mColorScale.data(),
                SLOT(setDataScaleType(QCPAxis::ScaleType)));
        connect(this, SIGNAL(gradientChanged(
                                 QCPColorGradient)), mColorScale.data(),
                SLOT(setGradient(QCPColorGradient)));
        connect(mColorScale.data(), SIGNAL(dataRangeChanged(
                                               QCPRange)), this,
                SLOT(setDataRange(QCPRange)));
        connect(mColorScale.data(), SIGNAL(gradientChanged(
                                               QCPColorGradient)), this,
                SLOT(setGradient(QCPColorGradient)));
...
void QCPColorMap::updateLegendIcon(Qt::TransformationMode transformMode,
    ...
    if (!mMapImage.isNull()) { // might still be null, e.g. if data is empty, so check here again
        bool mirrorX =
            (keyAxis()->orientation() ==
             Qt::Horizontal ? keyAxis() : valueAxis())->rangeReversed();
        bool mirrorY =
            (valueAxis()->orientation() ==
             Qt::Vertical ? valueAxis() : keyAxis())->rangeReversed();
        mLegendIcon =
...
void QCPColorMap::updateMapImage() {
    ...
    if (keyAxis->orientation() == Qt::Horizontal &&
        ...
        mMapImage = QImage(QSize(mMapData->keySize(),
                                 mMapData->valueSize()), QImage::Format_RGB32);
    }
    else if (keyAxis->orientation() == Qt::Vertical &&
             (mMapImage.size().width() != mMapData->valueSize() ||
              mMapImage.size().height() != mMapData->keySize())) {
        mMapImage = QImage(QSize(mMapData->valueSize(),
                                 mMapData->keySize()), QImage::Format_RGB32);
    }
    ...
    if (keyAxis->orientation() == Qt::Horizontal) {
        ...
        for (int line = 0; line < lineCount; ++line) {
            QRgb* pixels =
                reinterpret_cast<QRgb*>(mMapImage.scanLine(lineCount - 1 -
...
void QCPColorMap::draw(QCPPainter* painter) {
    ...
    if (mMapData->keySize() > 1) {
        halfSampleKey = 0.5 * mMapData->keyRange().size() /
    ...
    QRectF imageRect(coordsToPixels(
                         mMapData->keyRange().lower - halfSampleKey,
                         mMapData->valueRange().lower - halfSampleValue),
                     coordsToPixels(
                         mMapData->keyRange().upper + halfSampleKey,
    ...
    bool mirrorY =
        (valueAxis()->orientation() ==
         Qt::Vertical ? valueAxis() : keyAxis())->rangeReversed();
    bool smoothBackup = painter->renderHints().testFlag(
...
void QCPFinancial::setData(const QVector<double>& key,
    ...
    for (int i = 0; i < n; ++i) {
        mData->insertMulti(key[i],
                           QCPFinancialData(key[i], open[i], high[i], low[i],
                                            close[i]));
    }
}
...
QCPFinancialDataMap QCPFinancial::timeSeriesToOhlc(const QVector<double>& time,
    ...
    QCPFinancialData currentBinData(0, value.first(),
...
QCPRange QCPFinancial::getKeyRange(bool& foundRange,
                                   QCPAbstractPlottable::SignDomain inSignDomain)
const {
    QCPRange range;
    bool haveLower = false;
    bool haveUpper = false;
    ...
    if (keyAxis->orientation() == Qt::Horizontal) {
        for (QCPFinancialDataMap::const_iterator it = begin; it != end; ++it) {
            if (mSelected) {
                ...
                linePen = it.value().close >=
                          it.value().open ? mPenPositive : mPenNegative;
            }
            else {
                linePen = mPen;
            }
            ...
            painter->drawLine(QPointF(keyPixel - keyWidthPixels,
                                      openPixel), QPointF(keyPixel, openPixel));
            // draw close:
        ...
        for (QCPFinancialDataMap::const_iterator it = begin; it != end; ++it) {
            ...
            double keyWidthPixels = keyPixel - keyAxis->coordToPixel(
                it.value().key - mWidth * 0.5);                                          // sign of this makes sure open/close are on correct sides
            painter->drawLine(QPointF(openPixel,
    ...
    if (keyAxis->orientation() == Qt::Horizontal) {
        for (QCPFinancialDataMap::const_iterator it = begin; it != end; ++it) {
            ...
            painter->drawLine(QPointF(keyPixel,
                                      valueAxis->coordToPixel(it.value().high)),
                              QPointF(keyPixel,
                                      valueAxis->coordToPixel(qMax(it.value().
                                                                   open,
                                                                   it.value().
                                                                   close))));
            // draw low:
            painter->drawLine(QPointF(keyPixel,
                                      valueAxis->coordToPixel(it.value().low)),
                              QPointF(keyPixel,
                                      valueAxis->coordToPixel(qMin(it.value().
                                                                   open,
                                                                   it.value().
                                                                   close))));
            // draw open-close box:
            ...
            painter->drawRect(QRectF(QPointF(keyPixel - keyWidthPixels,
                                             closePixel),
                                     QPointF(keyPixel + keyWidthPixels,
                                             openPixel)));
        }
    }
    else { // keyAxis->orientation() == Qt::Vertical
        for (QCPFinancialDataMap::const_iterator it = begin; it != end; ++it) {
            ...
            painter->drawLine(QPointF(valueAxis->coordToPixel(it.value().high),
                                      keyPixel),
                              QPointF(valueAxis->coordToPixel(qMax(it.value().
                                                                   open,
                                                                   it.value().
                                                                   close)),
                                      keyPixel));
            // draw low:
            painter->drawLine(QPointF(valueAxis->coordToPixel(it.value().low),
                                      keyPixel),
                              QPointF(valueAxis->coordToPixel(qMin(it.value().
                                                                   open,
                                                                   it.value().
                                                                   close)),
                                      keyPixel));
            ...
            double keyWidthPixels = keyPixel - keyAxis->coordToPixel(
                it.value().key - mWidth * 0.5);
            painter->drawRect(QRectF(QPointF(closePixel,
                                             keyPixel - keyWidthPixels),
                                     QPointF(openPixel,
                                             keyPixel + keyWidthPixels)));
        }
...
const {
    ...
    if (keyAxis->orientation() == Qt::Horizontal) {
        for (it = begin; it != end; ++it) {
            ...
            double currentDistSqr =
                distSqrToLine(QPointF(keyPixel,
                                      valueAxis->coordToPixel(it.value().high)),
                              QPointF(keyPixel,
        ...
        for (it = begin; it != end; ++it) {
            ...
            // calculate distance to backbone:
            double currentDistSqr =
                distSqrToLine(QPointF(valueAxis->coordToPixel(it.value().high),
                                      keyPixel),
                              QPointF(valueAxis->coordToPixel(
                                          it.value().low),
                                      keyPixel), pos);
    ...
    if (keyAxis->orientation() == Qt::Horizontal) {
        ...
        for (it = begin; it != end; ++it) {
            ...
            if (boxKeyRange.contains(posKey) &&
                ...
                double keyPixel = keyAxis->coordToPixel(it.value().key);
                double highLineDistSqr =
                    distSqrToLine(QPointF(valueAxis->coordToPixel(it.value().
                                                                  high),
                                          keyPixel),
                                  QPointF(valueAxis->coordToPixel(qMax(it.value()
                                                                       .open,
                                          ...
                                          keyPixel), pos);
                double lowLineDistSqr =
                    distSqrToLine(QPointF(valueAxis->coordToPixel(it.value().low),
                                          keyPixel),
                                  QPointF(valueAxis->coordToPixel(qMin(it.value()
                                                                       .open,
                                                                       it.value()
...
double QCPItemStraightLine::selectTest(const QPointF& pos,
    ...
    return distToStraightLine(QVector2D(point1->pixelPoint()),
                              QVector2D(point2->pixelPoint() -
                                        point1->pixelPoint()), QVector2D(pos));
}
                                                       ...
                                                       const QRect& rect) const
{
    ...
    if (qFuzzyIsNull(vec.x())) { // line is vertical
        ...
        if (pointVectors.size() == 2) {
            result.setPoints(pointVectors.at(0).toPointF(), pointVectors.at(
                                 1).toPointF());
        }
            ...
            for (int i = 0; i < pointVectors.size() - 1; ++i) {
                for (int k = i + 1; k < pointVectors.size(); ++k) {
                    double distSqr =
                        (pointVectors.at(i) -
                         pointVectors.at(k)).lengthSquared();
                    if (distSqr > distSqrMax) {
...
void QCPItemLine::draw(QCPPainter* painter) {
    ...
    QLineF line =
        getRectClippedLine(startVec, endVec,
                           clipRect().adjusted(-clipPad, -clipPad, clipPad,
                                               clipPad));
    // paint visible segment, if existent:
    if (!line.isNull()) {
        painter->setPen(mainPen());
...
void QCPItemCurve::draw(QCPPainter* painter) {
    ...
    if (clip.intersects(cubicRect)) {
        ...
        painter->setBrush(Qt::SolidPattern);
        if (mTail.style() != QCPLineEnding::esNone) {
            mTail.draw(painter, QVector2D(
                           startVec), M_PI - cubicPath.angleAtPercent(
                           0) / 180.0 * M_PI);
        }
        if (mHead.style() != QCPLineEnding::esNone) {
            mHead.draw(painter, QVector2D(endVec), -cubicPath.angleAtPercent(
                           1) / 180.0 * M_PI);
        }
    }
}
...
double QCPItemText::selectTest(const QPointF& pos,
    ...
    QRect textRect = fontMetrics.boundingRect(0,
                                              ...
                                              mText);
    QRect textBoxRect = textRect.adjusted(-mPadding.left(),
                                          -mPadding.top(),
                                          mPadding.right(), mPadding.bottom());
    QPointF textPos = getTextDrawPoint(positionPixels,
                                       textBoxRect,
                                       mPositionAlignment);
...
void QCPItemText::draw(QCPPainter* painter) {
    ...
    QRect textBoxRect = textRect.adjusted(-mPadding.left(),
                                          -mPadding.top(),
                                          mPadding.right(), mPadding.bottom());
    QPointF textPos = getTextDrawPoint(QPointF(0,
                                               0), textBoxRect,
    ...
    textBoxRect.moveTopLeft(textPos.toPoint());
    double clipPad = mainPen().widthF();
    QRect boundingRect = textBoxRect.adjusted(-clipPad,
                                              -clipPad,
                                              clipPad,
                                              clipPad);
...
QPointF QCPItemText::anchorPixelPoint(int anchorId) const {
    ...
    QRect textRect = fontMetrics.boundingRect(0,
                                              ...
                                              Qt::TextDontClip | mTextAlignment,
                                              mText);
    QRectF textBoxRect = textRect.adjusted(-mPadding.left(),
                                           -mPadding.top(),
                                           mPadding.right(), mPadding.bottom());
    QPointF textPos = getTextDrawPoint(QPointF(0,
                                               0), textBoxRect,
...
QCPItemEllipse::QCPItemEllipse(QCustomPlot* parentPlot) :
    ...
    bottomRightRim(createAnchor(QLatin1String("bottomRightRim"),
                                aiBottomRightRim)),
    bottom(createAnchor(QLatin1String("bottom"), aiBottom)),
    bottomLeftRim(createAnchor(QLatin1String("bottomLeftRim"),
                               aiBottomLeftRim)),
    left(createAnchor(QLatin1String("left"), aiLeft)),
...
QPointF QCPItemEllipse::anchorPixelPoint(int anchorId) const {
    QRectF rect = QRectF(topLeft->pixelPoint(), bottomRight->pixelPoint());
    switch (anchorId) {
    case aiTopLeftRim:     return rect.center() +
               (rect.topLeft() - rect.center()) * 1 / qSqrt(2);
    case aiTop:            return (rect.topLeft() + rect.topRight()) * 0.5;
    case aiTopRightRim:    return rect.center() +
               (rect.topRight() - rect.center()) * 1 / qSqrt(2);
    case aiRight:          return (rect.topRight() + rect.bottomRight()) * 0.5;
    case aiBottomRightRim: return rect.center() +
               (rect.bottomRight() - rect.center()) * 1 / qSqrt(2);
    case aiBottom:         return (rect.bottomLeft() + rect.bottomRight()) *
               0.5;
    case aiBottomLeftRim:  return rect.center() +
...
double QCPItemTracer::selectTest(const QPointF& pos,
    ...
    case tsCircle:
    {
        if (clip.intersects(QRectF(center - QPointF(w, w),
                                   center + QPointF(w, w)).toRect())) {
            // distance to border:
...
void QCPItemTracer::draw(QCPPainter* painter) {
    ...
    case tsSquare:
    {
        if (clip.intersects(QRectF(center - QPointF(w, w),
                                   center + QPointF(w, w)).toRect())) {
            painter->drawRect(QRectF(center - QPointF(w, w),
                                     center + QPointF(w, w)));
        }
        break;
    }
